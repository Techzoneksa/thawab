...
const ROLES = [
  { id: "role-admin", name: "مدير النظام", description: "صلاحيات كاملة", permissions: ["*"] },
  {
    ...
    permissions: ["finance.*", "donations.*", "campaigns.*", "receipts.*", "donors.view", "reports.*", "audit.view", "documents.*"],
  },
  {
    id: "role-manager",
    name: "مدير تنفيذي",
    description: "إدارة البرامج والمشاريع",
    permissions: ["projects.*", "aid.*", "beneficiaries.*", "donations.*", "campaigns.*", "approvals.*", "*.view", "documents.*"],
  },
  {
    id: "role-data-entry",
    name: "مدخل بيانات",
