...
function ReceiptVoucherDetailPage() {
  const { id } = useParams({ from: "/finance/receipt-vouchers_/$id" });
  ...
  const back = () => nav({ to: "/finance/receipt-vouchers" });
  ...
  const buildDoc = (): DocumentDefinition => {
    ...
    return {
      ...
      entity: { name: v.payerName || "—" },
      meta: [
        { label: "الحالة", value: RV_STATUS[v.status]?.label || v.status },
        { label: "العملة", value: v.currency },
        {
          label: "الوجهة",
          value: d!.destination
            ? `${d!.destination.code} (${v.cashboxId ? "صندوق" : "بنك"})`
      ...
      signature: true,
      fileBase: `receipt-voucher-${v.voucherNumber}`,
    };
  };

  const share = async () => {
    const url = window.location.href;
    const title = `سند قبض ${d?.item?.voucherNumber || ""}`;
  ...
  return (
    <AppShell
      breadcrumb={["الرئيسية", "المالية", "سندات القبض", d?.item?.voucherNumber || "سند"]}
      title={`سند قبض: ${d?.item?.voucherNumber || ""}`}
      actions={
        <>
          ...
          <Card className="p-3 flex flex-wrap gap-1.5">
            {st === "draft" && can("finance.receipt.update_draft") && (
              <Btn
                variant="outline"
                onClick={() =>
                  nav({ to: "/finance/receipt-vouchers/$id/edit", params: { id } as any })
                }
              >
