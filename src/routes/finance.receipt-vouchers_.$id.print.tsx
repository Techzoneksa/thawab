...
import { getReceiptVoucher } from "@/lib/api/receipt-vouchers";
...
function PrintPage() {
  const { id } = useParams({ from: "/finance/receipt-vouchers_/$id/print" });
  const q = useQuery({
    queryKey: ["receipt-voucher", id, "print"],
    queryFn: () => getReceiptVoucher(id),
  });
  const d = q.data;
  ...
  if (!d) return <div className="p-8 text-center text-red-600">تعذّر جلب سند القبض</div>;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between border-b-2 border-black pb-3">
        <div>
          <div className="text-2xl font-extrabold">سند قبض</div>
          <div className="text-xs text-gray-600 mt-1">RECEIPT VOUCHER</div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-3 mt-5 text-sm">
        <Row label="استلمنا من" value={v.payerName || "—"} />
        <Row label="طريقة الاستلام" value={v.cashboxId ? "صندوق نقدي" : "حساب بنكي"} />
        <Row label="الوجهة" value={d.destination ? d.destination.code : "—"} />
      ...
      <table className="w-full mt-5 text-sm border border-black border-collapse">
        <thead>
          <tr className="bg-gray-100">
            <th className="border border-black p-1.5 text-right">الحساب (الطرف الدائن)</th>
