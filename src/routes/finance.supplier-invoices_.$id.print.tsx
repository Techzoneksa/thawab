...
import { getSupplierInvoice } from "@/lib/api/supplier-invoices";
...
function PrintPage() {
  const { id } = useParams({ from: "/finance/supplier-invoices_/$id/print" });
  const q = useQuery({
    queryKey: ["supplier-invoice", id, "print"],
  ...
  if (!d) return <div className="p-8 text-center text-red-600">تعذّر جلب فاتورة المورد</div>;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between border-b-2 border-black pb-3">
        <div>
          <div className="text-2xl font-extrabold">فاتورة مورد</div>
          <div className="text-xs text-gray-600 mt-1">SUPPLIER INVOICE</div>
        </div>
        <div className="text-left text-sm">
          <div>
            رقم الفاتورة الداخلي: <span className="font-mono font-bold">{v.invoiceNumber}</span>
          </div>
          <div>
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-3 mt-5 text-sm">
        ...
        <Row label="رقم فاتورة المورد" value={v.supplierInvoiceNumber || "—"} />
      ...
      <table className="w-full mt-5 text-sm border border-black border-collapse">
        ...
        <tbody>
          ...
          <tr className="font-bold bg-gray-50">
            ...
            <td className="border border-black p-1.5 text-left tabular-nums" colSpan={3}>
              {fmtSAR(v.subtotal)}
            </td>
          </tr>
          <tr className="font-bold bg-gray-50">
