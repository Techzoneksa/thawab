...
/** Shared full-page payment-voucher (سند صرف) form for create and draft-edit. */
export function PaymentVoucherForm({ voucherId }: { voucherId?: string }) {
  const isEdit = !!voucherId;
  const { user } = useAuth();
  const nav = useNavigate();
  const qc = useQueryClient();
  const canCreate = userCan(user, "finance.payment.create");
  const canEdit = userCan(user, "finance.payment.update_draft");
  ...
  const detailQ = useQuery({
    queryKey: ["payment-voucher", voucherId, "edit"],
  ...
  const done = () => {
    if (isEdit) nav({ to: "/finance/payment-vouchers/$id", params: { id: voucherId } as any });
    else nav({ to: "/finance/payment-vouchers" });
  };
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      return isEdit ? updatePaymentVoucher(body) : createPaymentVoucher(body);
    },
    onSuccess: () => {
      showToast(isEdit ? "تم حفظ المسودة" : "تم إنشاء السند", "success");
      qc.invalidateQueries({ queryKey: ["payment-vouchers"] });
      qc.invalidateQueries({ queryKey: ["payment-voucher", voucherId] });
      done();
    },
  ...
  const canSubmit = !!dest && !!f.payeeName.trim() && validLines.length > 0 && !mut.isPending;
  const notDraft = isEdit && detailQ.data && detailQ.data.item.status !== "draft";

  return (
    <AppShell
      breadcrumb={["الرئيسية", "المالية", "سندات الصرف", isEdit ? "تعديل" : "جديد"]}
      title={isEdit ? "تعديل سند صرف" : "سند صرف جديد"}
      actions={
        ...
        <div className="mx-auto max-w-3xl space-y-4 pb-6">
          ...
          <Card className="p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-semibold">حسابات الطرف المدين *</div>
