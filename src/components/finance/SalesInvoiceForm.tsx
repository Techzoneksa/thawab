...
import { createSalesInvoice, updateSalesInvoice, getSalesInvoice } from "@/lib/api/sales-invoices";
...
/** Shared full-page sales-invoice form for both create and edit (draft only). */
export function SalesInvoiceForm({ invoiceId }: { invoiceId?: string }) {
  ...
  const detailQ = useQuery({
    queryKey: ["sales-invoice", invoiceId, "edit"],
  ...
  const done = () => nav({ to: "/finance/sales-invoices" });
  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit) return updateSalesInvoice({ id: invoiceId, ...body });
  ...
  return (
    <AppShell
      breadcrumb={["الرئيسية", "المالية", "فواتير المبيعات", isEdit ? "تعديل" : "جديدة"]}
      title={isEdit ? "تعديل فاتورة مبيعات" : "فاتورة مبيعات جديدة"}
      actions={
