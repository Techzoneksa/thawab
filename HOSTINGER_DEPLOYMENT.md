...
## متغيرات البيئة وقاعدة البيانات

المشروع يعمل الآن على قاعدة بيانات **PostgreSQL** حقيقية (Supabase)، ويتطلّب
ضبط متغيّر البيئة التالي في إعدادات التطبيق (Hostinger → Node.js app → Environment
variables، أو ملف `.env` في مجلد الإعداد):

```
DATABASE_URL='postgresql://<user>:<password>@<host>:6543/postgres?pgbouncer=true'
```

> استخدم رابط الـ **pooler** (المنفذ 6543) مع `?pgbouncer=true`. إذا احتوت كلمة
> المرور على رموز خاصة (`! @ # / ?`) رمّزها (URL-encode) أو اختر كلمة مرور
> بحروف وأرقام فقط.

### الترحيلات (Migrations)

- عند الإقلاع يحاول التطبيق تطبيق الترحيلات تلقائياً إذا وجد مجلد `drizzle`
  (يُشحن مع الحزمة عبر `postbuild` إلى `server/drizzle`).
- الطريقة الموثوقة يدوياً: `npm run db:migrate` من نسخة المصدر الكاملة مقابل
  قاعدة الإنتاج.
- تتبُّع الترحيلات المطبّقة في الجدول `drizzle.__drizzle_migrations`.

### التهيئة الأولية (Seed) — إلزامية مرة واحدة

بعد أول ترحيل، شغّل البذرة لإنشاء دليل الحسابات + مفاتيح النظام المحاسبية +
حساب المدير:
...
تُنشئ حساب المدير `admin@thawab.local` بكلمة مرور عشوائية تُطبع مرة واحدة
(غيّرها بعد أول دخول). إعادة تشغيل `seed` آمنة (idempotent).
